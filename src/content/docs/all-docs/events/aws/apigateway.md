...
To create HTTP endpoints as Event sources for your AWS Lambda Functions, use the OSS Serverless's easy AWS API Gateway Events syntax.
...
By default, the OSS Serverless deploys your REST API using the EDGE endpoint configuration. If you would like to use the REGIONAL or PRIVATE configuration, set the `endpointType` parameter in your `provider` block.
