...
provider:
  ...
  # Version of hashing algorithm used by OSS Serverless for function packaging
...
OSS Serverless needs a S3 bucket to store artifacts for deploying. That bucket is automatically created and managed by Serverless, but you can configure it explicitly if needed:

```yaml
provider:
  # The S3 prefix under which deployed artifacts are stored (default: serverless)
  deploymentPrefix: serverless
  # Configure the S3 bucket used by OSS Serverless to deploy code packages to Lambda
  deploymentBucket:
  ...
  alb:
    ...
    authorizers:
      ...
      mySecondAuth:
        ...
        # Required
        tokenEndpoint: 'https://somewhere.org'
        # Required
...
provider:
  logs:
    ...
    # Can only be configured if the API is created by OSS Serverless
    httpApi:
  ...
  Outputs:
    UsersTableArn:
      ...
      Export:
        ...
        # and https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/outputs-section-structure.html for documentation on use.
