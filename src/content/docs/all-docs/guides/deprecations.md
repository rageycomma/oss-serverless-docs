---
title: OSS Serverless Deprecations
---
...
An alternative to free-form CLI options is to use [environment variables](./variables.md#referencing-environment-variables). Another option is to use [the `--param` option](./parameters.md#cli-parameters) introduced in OSS Serverless **v3.3.0**:
...
A more robust and powerful variable resolver engine was introduced (disabled by default) in OSS Serverless v2. It is used by default in v3.
...
_Note: Starting with v3.0.0, OSS Serverless will switch exclusively to a new variables resolver. If you see this deprecation please upgrade to latest v2 release of OSS Serverless, as that will provide a more accurate insight on planned changes._
...
_Note: Originally, support for the legacy IAM settings format was scheduled to be dropped in v3. However, it's no longer the case. If you see this deprecation notice please upgrade to the latest version of OSS Serverless v2._
