---
title: OSS Serverless Concepts
---

The OSS Serverless helps you develop and deploy AWS Lambda functions, along with the AWS infrastructure resources they require. It's a CLI that offers structure, automation and best practices out-of-the-box, allowing you to focus on building sophisticated, event-driven, serverless architectures, comprised of [Functions](#functions) and [Events](#events).

The OSS Serverless is different from other application frameworks because:
...
When you configure an event on a Lambda function, OSS Serverless will automatically create the infrastructure needed for that event (e.g. an API Gateway endpoint) and configure your functions to listen to it.
...
- Anything that can be defined in CloudFormation is supported by the OSS Serverless

OSS Serverless can deploy functions and their events, but also AWS resources.
...
While OSS Serverless is language-agnostic, projects written in Node.js can benefit from using the same language all around. When using JavaScript or TypeScript, the file must export the configuration as a JS object, for example:
