...
The OSS Serverless gives you an intuitive way to reference multiple variables as a fallback strategy in case one of the variables is missing. This way you'll be able to use a default value from a certain source, if the variable from another source is missing.
...
Otherwise OSS Serverless has no implied understanding of them and does not try to resolve them on its own.
