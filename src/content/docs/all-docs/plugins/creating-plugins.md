...
The simplest way to create a OSS Serverless plugin is to write a JavaScript file:
...
It is also a good practice to add `serverless` to the `peerDependencies` section. That ensures that your plugin runs only with the OSS Serverless versions it supports.
